...
void UpdateVelocity(Map *map, const Input *input) {
    ...
    int tx =
        Clamp((int)(map->player.position.x / TILE_SIZE), 0, map->columns - 1);
    ...
    if (input->buttons_pressed & JUMP_PRESSED) {
        // Allow jumping if we are on the floor (or within 2 pixels for "coyote
        // time")
        if (map->player.altitude <= floorY + 2.0f) {
            map->player.state = JUMPING_STATE;
            map->player.vertical_velocity =
                (500.0f + (float)PLAYER->stats.current[STAT_AEROBICS]);
        }
    }

    if (input->buttons_pressed & MOVEMENT_PRESSED) {
        float length = (input->dir.x != 0 && input->dir.y != 0) ? 0.707f : 1.0f;
        map->player.velocity.x =
            input->dir.x *
            (PLAYER->speed + (float)PLAYER->stats.current[STAT_SPEED]) * length;
        map->player.velocity.y =
            input->dir.y *
            (PLAYER->speed + (float)PLAYER->stats.current[STAT_SPEED]) * length;
    } else {
...
bool CheckCollision(Map *map, Vector2 nextPos) {
    ...
    if (cur_x < 0 || cur_x >= map->columns || cur_y < 0 || cur_y >= map->rows)
        return true;
    ...
    int checkX[] = {(int)(foot_left / TILE_SIZE),
                    (int)(foot_right / TILE_SIZE)};
    int checkY[] = {(int)(foot_top / TILE_SIZE),
                    (int)(foot_bottom / TILE_SIZE)};

    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            int cx = Clamp(checkX[i], 0, map->columns - 1);
            int cy = Clamp(checkY[j], 0, map->rows - 1);
            float targetHeight = (float)map->grid[cy][cx].height;
            bool is_blocking = TILE_REGISTRY[map->grid[cy][cx].type].is_blocking;

            if (is_blocking)
                return true;

            // STEP HEIGHT LOGIC:
            // If the tile is more than 1.5 units higher than our CURRENT
            // feet...
